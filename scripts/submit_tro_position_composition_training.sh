...
MODEL_ROOT="/scratch/prj/eng_demt_robot_learning/trained_models/tro_position_stage2_composition"
LOG_DIR="${PROJECT_DIR}/dataset/tro_position_stage2_composition/logs"
...
for canonical_seed in 1 2 3 4 5; do
  ...
  for condition in "${conditions[@]}"; do
    config="${PROJECT_DIR}/experiments/tro_stage2_position_composition/configs/training/seed${canonical_seed}/${condition}.json"
    run_name="composition_seed${canonical_seed}_source${source_seed}_${condition}"
    ...
    if find "${condition_root}/${run_name}" -type f -name 'model_epoch_*.pth' -print -quit 2>/dev/null | grep -q .; then
      printf '[ERROR] Refusing duplicate training: seed%s/%s\n' \
        "${canonical_seed}" "${condition}" >&2
      exit 1
    fi

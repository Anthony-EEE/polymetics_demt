...
CANDIDATE_SOURCE_PATHS = {
    1: (
        AXIAL_MANIFESTS / "paired_latents"
        / "seed1_fallback_whole_slot.json"
    ),
    2: AXIAL_MANIFESTS / "paired_latents" / "seed2.json",
    3: AXIAL_MANIFESTS / "paired_latents" / "seed3.json",
    4: (
        ROOT / "experiments" / "mvp0_position_event" / "manifests"
        / "paired_latents_b1.json"
    ),
    5: (
        AXIAL_MANIFESTS / "paired_latents"
        / "seed5_fallback_whole_slot.json"
    ),
}
...
def progress_v3_path(run_root):
    return Path(run_root) / "collection_progress_v3.json"


def candidate_staging_path(run_root, decision_index, candidate_index):
    return (
        Path(run_root) / ".paired_staging_v3"
        / f"decision_{decision_index:03d}_candidate_{candidate_index:04d}"
    )


def accepted_candidate_bundle_path(
    run_root, decision_index, candidate_index
):
    return (
        Path(run_root) / "accepted_bundles_v3"
        / f"decision_{decision_index:03d}_candidate_{candidate_index:04d}"
    )


def rejected_candidate_bundle_path(
    run_root, decision_index, candidate_index
):
    return (
        Path(run_root) / "scientific_candidates_v3"
        / f"decision_{decision_index:03d}_candidate_{candidate_index:04d}"
    )


def candidate_record(result_path):
    payload = read_json(result_path)
    required = {
        "decision_index", "demo_index", "candidate_index",
        "candidate_runtime_seed", "candidate_source", "latent",
        "condition_results",
    }
    if payload.get("protocol_id") != V3_PROTOCOL_ID:
        raise ValueError(f"Unexpected v3 candidate protocol: {result_path}")
    if not required.issubset(payload):
        raise ValueError(f"Incomplete v3 candidate record: {result_path}")
    if list(payload["condition_results"]) != list(NEW_CONDITIONS):
        raise ValueError(f"Candidate condition order changed: {result_path}")
    if int(payload["latent"]["candidate_index"]) != int(
        payload["candidate_index"]
    ):
        raise ValueError(f"Candidate latent identity changed: {result_path}")
    if int(payload["latent"]["runtime_seed"]) != int(
        payload["candidate_runtime_seed"]
    ):
        raise ValueError(f"Candidate runtime seed changed: {result_path}")
    return payload


def finalize_candidate(run_root, staging, result):
    """Archive or accept exactly one v3 four-corner candidate atomically."""
    run_root = Path(run_root)
    staging = Path(staging)
    ...
    if all(successes):
        destination = accepted_candidate_bundle_path(
            run_root, decision_index, candidate_index
        )
        if staging.exists():
            destination.parent.mkdir(parents=True, exist_ok=True)
        ...
        return "accepted", destination
    destination = rejected_candidate_bundle_path(
        run_root, decision_index, candidate_index
    )
    if staging.exists():
        destination.parent.mkdir(parents=True, exist_ok=True)
...
def generated_candidate(seed, candidate_index):
    """Regenerate an exact candidate from the canonical deterministic stream."""
    seed = int(seed)
    candidate_index = int(candidate_index)
    if seed not in SEEDS or candidate_index < 0:
        raise ValueError("Invalid canonical seed or candidate index")
    collection_seed = int(SEEDS[seed]["collection"])
    ...
    for index in range(candidate_index + 1):
        row = {
            ...
            "accepted_candidate_id":
                f"seed{seed}_candidate_{index:03d}",
...
def assert_candidate_stream_identity(actual, expected):
    keys = (
        "candidate_index", "start_angle", "start_radial_quantile",
        "approach_angle", "approach_radial_quantile", "collection_seed",
        "runtime_seed",
    )
    for key in keys:
        if actual.get(key) != expected.get(key):
            raise ValueError(
                f"Candidate stream identity changed at key {key}: "
                f"{actual.get(key)!r} != {expected.get(key)!r}"
            )


def verify_candidate_stream_sources():
    """Verify every persisted 0..159 candidate against its generator."""
    rows = []
    for seed in SEEDS:
        path = CANDIDATE_SOURCE_PATHS[seed]
        payload = read_json(path)
        source = payload["candidates"]
        if len(source) != 160:
            raise ValueError(f"seed{seed}: source pool is not 160 candidates")
        for index, actual in enumerate(source):
            assert_candidate_stream_identity(
                actual, generated_candidate(seed, index)
            )
        rows.append({
            "canonical_seed": seed,
            ...
            "sha256": sha256(path),
            "verified_candidate_indices": [0, 159],
            "verified_records": 160,
        })
    return rows


def v3_candidate(seed, candidate_index):
    """Return a source-verified candidate, extending the stream as needed."""
    candidate_index = int(candidate_index)
    if candidate_index < 160:
        actual = dict(
            read_json(CANDIDATE_SOURCE_PATHS[int(seed)])["candidates"][
                candidate_index
            ]
        )
        generated = generated_candidate(seed, candidate_index)
        assert_candidate_stream_identity(actual, generated)
        actual.setdefault(
            "accepted_candidate_id",
            generated["accepted_candidate_id"],
        )
        return actual
    return generated_candidate(seed, candidate_index)


def v3_latent(latent, candidate_source, old_anchor_paired):
    value = dict(latent)
    value["composition_v3"] = {
        "protocol_id": V3_PROTOCOL_ID,
        "candidate_source": candidate_source,
        "candidate_runtime_seed": int(latent["runtime_seed"]),
        "same_candidate_across_four_corners": True,
        "old_anchor_paired": bool(old_anchor_paired),
    }
    return value


def next_v3_candidate(progress, frozen_records):
    cursor = int(progress["frozen_cursor"])
    if cursor < len(frozen_records):
        latent = dict(frozen_records[cursor])
        return latent, {
            "phase": "remaining_original_frozen_order",
            "frozen_position": cursor,
            "old_anchor_paired": True,
        }
    candidate_index = int(progress["next_extension_candidate_index"])
    latent = v3_candidate(progress["canonical_seed"], candidate_index)
    return latent, {
...
def advance_v3_candidate(progress, candidate_source):
    if candidate_source["phase"] == "remaining_original_frozen_order":
        progress["frozen_cursor"] = int(progress["frozen_cursor"]) + 1
    else:
        progress["next_extension_candidate_index"] = (
            int(progress["next_extension_candidate_index"]) + 1
        )
...
def migrate_v2_progress(seed):
    """Create a v3 ledger while preserving the complete v1/v2 history."""
    seed = int(seed)
    run_root = collection_root(seed, False)
    old_path = progress_v2_path(run_root)
    new_path = progress_v3_path(run_root)
    frozen_records, latent_info = accepted_latents(seed)
    frozen_ids = [int(row["candidate_index"]) for row in frozen_records]
    if new_path.exists():
        progress = read_json(new_path)
        if (
            progress["protocol_id"] != V3_PROTOCOL_ID
            or progress["v2_progress_sha256"] != sha256(old_path)
            or progress["original_frozen_candidate_indices"] != frozen_ids
        ):
            raise ValueError(f"v3 progress invariant changed: {new_path}")
        return progress

    old = read_json(old_path)
    if old["protocol_id"] != PROTOCOL_ID:
        raise ValueError(f"Unexpected v2 progress protocol: {old_path}")
    completed = [int(value) for value in old["accepted_candidate_indices"]]
    if completed != frozen_ids[:len(completed)]:
        raise ValueError(f"v2 accepted order changed: {old_path}")
    accepted_bundles = []
    accepted_values = []
    for demo_index, (candidate_index, bundle) in enumerate(
        zip(completed, old["accepted_bundles"])
    ):
        latent = dict(frozen_records[demo_index])
        if (
            int(bundle["demo_index"]) != demo_index
            or int(bundle["candidate_index"]) != candidate_index
        ):
            raise ValueError(f"v2 accepted bundle order changed: {old_path}")
        accepted_values.append(latent)
        accepted_bundles.append({
            **bundle,
            "candidate_source": {
                "phase": "original_frozen_order_migrated_from_v2",
                "frozen_position": demo_index,
                "old_anchor_paired": True,
            },
            "old_anchor_paired": True,
    ...
    attempts = list(old["scientific_attempts"])
    rejected = []
    frozen_cursor = len(completed)
    if attempts:
        expected_candidate = int(frozen_records[frozen_cursor]["candidate_index"])
        keys = {
            (int(row["demo_index"]), int(row["candidate_index"]))
            for row in attempts
        }
        if keys != {(len(completed), expected_candidate)}:
            raise ValueError(
                f"v2 scientific attempts are not one frontier candidate: {old_path}"
            )
        rejected.append({
            "decision_index": len(completed),
            "demo_index_at_rejection": len(completed),
            "candidate_index": expected_candidate,
            "accepted_candidate_id":
                frozen_records[frozen_cursor]["accepted_candidate_id"],
            "candidate_runtime_seed":
                int(frozen_records[frozen_cursor]["runtime_seed"]),
            "latent": dict(frozen_records[frozen_cursor]),
            "candidate_source": {
                "phase": "original_frozen_order_rejected_after_v2_audit",
                "frozen_position": frozen_cursor,
                "old_anchor_paired": True,
            },
            "old_anchor_paired": True,
            "failed_conditions_union": sorted({
                condition
                for row in attempts
                for condition in row["failed_conditions"]
            }),
            "preserved_v2_scientific_attempt_count": len(attempts),
            "preserved_v2_attempt_indices": [
                int(row["attempt_index"]) for row in attempts
            ],
            "preserved_v2_attempt_archives": [
                {
                    "path": row["archived_path"],
                    "tree_sha256": row["archive_tree_sha256"],
                }
                for row in attempts
            ],
            "source": "v3_migration_of_v2_deterministic_retry_blocker",
        })
        frozen_cursor += 1
    ...
    progress = {
        ...
        "execution_conditions": list(NEW_CONDITIONS),
        **latent_info,
        "original_frozen_candidate_indices": frozen_ids,
        "original_frozen_count": 30,
        "frozen_cursor": frozen_cursor,
        "extension_start_candidate_index": max(frozen_ids) + 1,
        "next_extension_candidate_index": max(frozen_ids) + 1,
        "accepted_candidate_indices": completed,
        "accepted_latents": accepted_values,
        "accepted_bundles": accepted_bundles,
        "rejected_candidates": rejected,
        "infrastructure_retries": [],
        "preserved_v2_infrastructure_retry_count":
            len(old["infrastructure_retries"]),
        "v2_progress_path": str(old_path.resolve()),
        "v2_progress_sha256": sha256(old_path),
        "v2_progress_preserved": True,
        "protocol_blocker": False,
    }
    write_json(new_path, progress)
    return progress


def freeze_protocol_v3(_args):
    amendment_path = MANIFESTS / "protocol_amendment_v3.json"
    amendment = read_json(amendment_path)
    if (
        amendment["protocol_id"] != V3_PROTOCOL_ID
        or amendment["status"] != "authorized_ready_to_implement"
    ):
        raise ValueError("Protocol v3 amendment is not authorised")
    source_verification = verify_candidate_stream_sources()
    stream_payload = {
        "schema_version": 1,
        "protocol_id": V3_PROTOCOL_ID,
        "status": "frozen",
        "generator": amendment["deterministic_candidate_stream"],
        "seeds": [],
    }
    for seed, source in zip(SEEDS, source_verification):
        frozen, _ = accepted_latents(seed)
        frozen_ids = [int(row["candidate_index"]) for row in frozen]
        stream_payload["seeds"].append({
            **source,
            "original_frozen_candidate_indices": frozen_ids,
            "original_frozen_count": 30,
            "extension_start_candidate_index": max(frozen_ids) + 1,
            "extension_order": "ascending candidate_index without limit",
        })
    stream_path = MANIFESTS / "candidate_stream_v3.json"
    write_frozen_json(stream_path, stream_payload)

    test_path = MANIFESTS / "test_evidence_v3.json"
    test_evidence = read_json(test_path)
    if (
        test_evidence.get("status") != "passed"
        or not test_evidence.get("all_required_v3_semantics_covered")
    ):
        raise ValueError("Protocol v3 test evidence is not complete")
    snapshots = {
        "collector": (
            EXPERIMENT / "implementation_snapshots"
            / "tro_position_composition_v3.py"
        ),
        "tests": (
            EXPERIMENT / "implementation_snapshots"
            / "test_tro_position_composition_v3.py"
        ),
    }
    ...
    migrations = []
    for seed in SEEDS:
        progress = migrate_v2_progress(seed)
        migrations.append({
            "canonical_seed": seed,
            "v2_progress_path": progress["v2_progress_path"],
            "v2_progress_sha256": progress["v2_progress_sha256"],
            "v3_progress_path": str(
                progress_v3_path(collection_root(seed, False)).resolve()
            ),
            "v3_progress_sha256_at_migration": sha256(
                progress_v3_path(collection_root(seed, False))
            ),
            "retained_complete_bundles":
                len(progress["accepted_bundles"]),
            "rejected_v2_frontier_candidates":
                len(progress["rejected_candidates"]),
            "next_frozen_position": int(progress["frozen_cursor"]),
            "extension_start_candidate_index":
                int(progress["extension_start_candidate_index"]),
        })
    preserved = {}
    for name in (
        "protocol_blocker.json",
    ...
    payload = {
        ...
        "status": "implemented_tested_frozen",
        "authority": {
            "plan": str((EXPERIMENT / "PLAN.md").resolve()),
            "user_confirmation_date": "2026-07-26",
        },
        "amendment_path": str(amendment_path.resolve()),
        "amendment_sha256": sha256(amendment_path),
        "candidate_stream_path": str(stream_path.resolve()),
        "candidate_stream_sha256": sha256(stream_path),
        "collection": {
            "target_accepted_bundles_per_seed": 30,
            "accepted_unit": "atomic_four_corner_candidate_bundle",
            "new_conditions": list(NEW_CONDITIONS),
            "candidate_failure_action":
                "archive all four corners and advance candidate stream",
            "infrastructure_failure_action":
                "clean retry identical candidate without advancing",
            "condition_specific_selection": False,
            "scientific_candidate_limit": None,
        },
        "progress_migration": migrations,
        "test_evidence": {
            **test_evidence,
        ...
        "preserved_v1_v2_evidence": preserved,
        "immutable_anchors_revalidated": True,
    }
    write_frozen_json(MANIFESTS / "protocol_v3.json", payload)
    print(json.dumps({
        "status": payload["status"],
        "protocol_id": V3_PROTOCOL_ID,
...
def run_candidate_v3(
    condition, latent, candidate_source, output_dir, sample_hz,
    playback_speed
):
    start_radius, approach_radius = CONDITIONS[condition]
    value = v3_latent(
        latent, candidate_source["phase"],
        candidate_source["old_anchor_paired"],
    )
    random.seed(int(value["runtime_seed"]))
    np.random.seed(int(value["runtime_seed"]))
    sim = PandaSim(gui=False, output_dir=output_dir, sample_hz=sample_hz)
    sim.playback_speed = playback_speed
    try:
        sim.setup()
...
def collect_v3(args):
    ...
    if bool(args.smoke) or target != 30:
        raise ValueError(
            "Protocol v3 is formal target-30 collection only; "
            "the completed v1 smoke remains immutable"
        )
    protocol = read_json(MANIFESTS / "protocol_v3.json")
    if (
        protocol["protocol_id"] != V3_PROTOCOL_ID
        or protocol["status"] != "implemented_tested_frozen"
    ):
        raise RuntimeError("Formal collection is gated by frozen protocol v3")
    verify_candidate_stream_sources()
    frozen_records, latent_info = accepted_latents(seed)
    run_root = collection_root(seed, False)
    raw_root = run_root / "raw"
    progress_path = progress_v3_path(run_root)
    progress = read_json(progress_path)
    invariants = (
        progress["protocol_id"] == V3_PROTOCOL_ID
        and int(progress["canonical_seed"]) == seed
        ...
        and progress["paired_latent_manifest_sha256"]
        == latent_info["paired_latent_manifest_sha256"]
        and progress["original_frozen_candidate_indices"]
        == [int(row["candidate_index"]) for row in frozen_records]
        and int(progress["extension_start_candidate_index"])
        == max(int(row["candidate_index"]) for row in frozen_records) + 1
    )
    if not invariants:
        raise ValueError(f"Collection resume invariant changed: {progress_path}")

    completed = [int(v) for v in progress["accepted_candidate_indices"]]
    if (
        len(completed) != len(progress["accepted_latents"])
        or len(completed) != len(progress["accepted_bundles"])
        or len(completed) > target
        or len(set(completed)) != len(completed)
    ):
        raise ValueError(f"Invalid accepted state: {progress_path}")
    for condition in NEW_CONDITIONS:
        root = raw_root / condition
        demos = list(root.glob("demo_*")) if root.exists() else []
        if len(demos) != len(completed):
            raise RuntimeError(
                f"seed{seed}/{condition}: {len(demos)} demos != "
                f"{len(completed)} accepted bundles"
            )

    while len(completed) < target:
        latent, candidate_source = next_v3_candidate(
            progress, frozen_records
        )
        candidate_index = int(latent["candidate_index"])
        demo_index = len(completed)
        decision_index = (
            len(progress["accepted_bundles"])
            + len(progress["rejected_candidates"])
        )
        staging = candidate_staging_path(
            run_root, decision_index, candidate_index
        )
        accepted_path = accepted_candidate_bundle_path(
            run_root, decision_index, candidate_index
        )
        rejected_path = rejected_candidate_bundle_path(
            run_root, decision_index, candidate_index
        )
        result = None
        if accepted_path.is_dir():
            result = candidate_record(
                accepted_path / "candidate_result.json"
            )
            disposition, archive = finalize_candidate(
                run_root, staging, result
            )
        elif rejected_path.is_dir():
            result = candidate_record(
                rejected_path / "candidate_result.json"
            )
            disposition, archive = finalize_candidate(
                run_root, staging, result
            )
            ...
            staging.exists()
            and (staging / "candidate_result.json").is_file()
        ):
            result = candidate_record(
            ...
            if staging.exists():
                retry_index = len(progress["infrastructure_retries"])
                archive = (
                    run_root / "infrastructure_attempts_v3"
                    / f"resume_decision_{decision_index:03d}_candidate_"
                      f"{candidate_index:04d}_{retry_index:03d}"
                )
                archive.parent.mkdir(parents=True, exist_ok=True)
                staging.rename(archive)
                progress["infrastructure_retries"].append({
                    ...
                    "archived_path": str(archive.resolve()),
                    "archive_tree_sha256": tree_sha256(archive),
                    "identical_candidate_clean_retry": True,
                })
                write_json(progress_path, progress)
            results = {}
            try:
                for condition in NEW_CONDITIONS:
                    ...
                    success, details = run_candidate_v3(
                        condition, latent, candidate_source, output,
                        float(args.sample_hz), float(args.playback_speed),
                    )
                    results[condition] = {
                        "success": bool(success), "details": details
                ...
                archive = (
                    run_root / "infrastructure_attempts_v3"
                    / f"exception_decision_{decision_index:03d}_candidate_"
                      f"{candidate_index:04d}_{retry_index:03d}"
                )
                if staging.exists():
                    archive.parent.mkdir(parents=True, exist_ok=True)
            ...
            write_json(staging / "candidate_result.json", result)
            disposition, archive = finalize_candidate(
                run_root, staging, result
            )

        if (
            int(result["decision_index"]) != decision_index
            or int(result["demo_index"]) != demo_index
            or int(result["candidate_index"]) != candidate_index
        ):
            raise ValueError("Recovered v3 candidate does not match ledger state")
        assert_candidate_stream_identity(result["latent"], latent)
        if result["candidate_source"] != candidate_source:
            raise ValueError("Recovered v3 candidate source changed")

        if disposition == "scientific_failure":
            if not any(
                int(row["decision_index"]) == decision_index
                for row in progress["rejected_candidates"]
            ):
                progress["rejected_candidates"].append({
                    **result,
                    ...
                    "archive_tree_sha256": tree_sha256(archive),
                    "old_anchor_paired":
                        bool(candidate_source["old_anchor_paired"]),
                    "source":
                        "protocol_v3_atomic_four_corner_candidate",
                })
                advance_v3_candidate(progress, candidate_source)
                write_json(progress_path, progress)
            print(
                f"seed{seed}: rejected candidate {candidate_index} "
                f"as atomic bundle; failed={result['failed_conditions']}; "
                "advancing candidate stream",
                flush=True,
            )
            continue
        ...
        accepted_record = {
            **result,
            ...
            "old_anchor_paired": bool(
                candidate_source["old_anchor_paired"]
            ),
            "source": "protocol_v3_atomic_four_corner_candidate",
        }
        if not any(
            int(row["demo_index"]) == demo_index
            for row in progress["accepted_bundles"]
            ...
            progress["accepted_bundles"].append(accepted_record)
            progress["accepted_latents"].append(dict(latent))
            completed.append(candidate_index)
            progress["accepted_candidate_indices"] = completed
            advance_v3_candidate(progress, candidate_source)
            write_json(progress_path, progress)
        print(
            f"seed{seed}: accepted candidate {candidate_index} "
            f"({len(completed)}/{target}); source="
            f"{candidate_source['phase']}",
            flush=True,
        )
    validate_raw(argparse.Namespace(
        canonical_seed=seed, target=target, smoke=False
...
def validate_raw(args):
    ...
    if args.smoke:
        ...
        progress_protocol = V1_PROTOCOL_ID
    else:
        v3_path = progress_v3_path(run_root)
        progress = read_json(v3_path)
        if progress["protocol_id"] != V3_PROTOCOL_ID:
            raise ValueError(f"Formal progress is not protocol v3: {v3_path}")
        records = list(progress["accepted_latents"])
        _, latent_info = accepted_latents(seed)
        progress_protocol = V3_PROTOCOL_ID
    accepted = [int(v) for v in progress["accepted_candidate_indices"]]
    expected = [int(row["candidate_index"]) for row in records]
    if accepted != expected:
        raise ValueError(f"seed{seed}: exact accepted IDs changed")
    if len(records) != target or len(set(accepted)) != target:
        raise ValueError(f"seed{seed}: expected exactly {target} unique accepts")
    accepted_by_demo = {}
    if not args.smoke:
    ...
    for condition in NEW_CONDITIONS:
        ...
        for demo_index, demo in enumerate(demos):
            ...
            if not args.smoke:
                ...
                expected_seed = int(
                    bundle.get(
                        "candidate_runtime_seed",
                        bundle.get("attempt_runtime_seed"),
                    )
                )
                ...
                if bundle["source"].startswith("protocol_v3"):
                    provenance = metadata["paired_latent"].get("composition_v3")
                    expected_provenance = {
                        "protocol_id": V3_PROTOCOL_ID,
                        "candidate_source":
                            bundle["candidate_source"]["phase"],
                        "candidate_runtime_seed": expected_seed,
                        "same_candidate_across_four_corners": True,
                        "old_anchor_paired":
                            bool(bundle["old_anchor_paired"]),
                    }
                    if provenance != expected_provenance:
                        errors.append(f"{demo}: v3 provenance mismatch")
            start_radius, approach_radius = CONDITIONS[condition]
        ...
        dataset_rows.append({
            ...
            "accepted_old_anchor_paired": [
                bool(row.get("old_anchor_paired", True))
                for row in progress.get("accepted_bundles", [])
            ] if not args.smoke else None,
            "old_anchor_paired_count": (
                sum(
                    bool(row.get("old_anchor_paired", True))
                    for row in progress["accepted_bundles"]
                ) if not args.smoke else target
            ),
            "raw_path": str(raw.resolve()),
    ...
    report = {
        ...
        "scientific_attempt_count": (
            None if args.smoke else len(progress["rejected_candidates"])
        ),
        "preserved_v2_scientific_attempt_count": (
            None if args.smoke else sum(
                int(row.get("preserved_v2_scientific_attempt_count", 0))
                for row in progress["rejected_candidates"]
            )
        ),
        ...
        "old_anchor_paired_accept_count": (
            None if args.smoke else sum(
                bool(row.get("old_anchor_paired", True))
                for row in progress["accepted_bundles"]
            )
...
def rollout_pairing_group(surface, condition):
    if surface == "common_absolute":
...
def strict_merge(_args):
    ...
    if len(cp) != 45:
        raise ValueError("Checkpoint manifest does not contain 45 unique slots")
    for key, row in cp.items():
        if sha256(row["checkpoint_path"]) != row["checkpoint_sha256"]:
            raise ValueError(f"Checkpoint hash changed before merge: {key}")
    reports = {}
    combined_index = {"schema_version": 1, "protocol_id": V3_PROTOCOL_ID}
    for surface, banks in (
            ...
            for rollout_seed in ROLLOUT_SEEDS:
                for bank in banks:
                    ...
                    for condition in CONDITION_ORDER:
                        ...
                        pairing_group = rollout_pairing_group(
                            surface, condition
                        )
                        if pairing_group not in reference_specs:
                            reference_specs[pairing_group] = specs
                        elif specs != reference_specs[pairing_group]:
                            raise ValueError(
                                f"{surface}/seed{seed}/rollout{rollout_seed}/"
                                f"{bank}/{pairing_group}: pairing mismatch"
                            )
                    ...
                    for pairing_group in reference_specs:
                        pairing[
        ...
        report = {
            ...
            "pairing_scope": (
                "all nine conditions on identical absolute states"
                if surface == "common_absolute"
                else "same-Start conditions on identical absolute states"
            ),
            "strict_pairing": all(pairing.values()),
...
def parse_args():
    ...
    commands.add_parser("freeze-protocol-v3").set_defaults(
        func=freeze_protocol_v3
    )
    commands.add_parser("audit-feasibility").set_defaults(func=feasibility_audit)
    for name, func in (
        ("collect", collect),
        ("collect-v3", collect_v3),
        ("validate-raw", validate_raw),
    ):

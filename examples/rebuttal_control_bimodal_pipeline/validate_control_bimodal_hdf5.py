#!/usr/bin/env python3
"""Validate Control-Bimodal HDF5 payloads, source equality, provenance, and loaders."""
...
def main() -> None:
    ...
    for source_id, record in source_records.items():
        ...
        if actual != record["hdf5"]["sha256"]:
            raise ValueError(f"Frozen Control HDF5 changed for {source_id}")
    ...
    if not build_manifest_path.is_file():
        ...
        if (
            ...
            or build_manifest.get("counts", {}).get("underlying_unique_control_trajectories") != 300
    ...
    report = {
        ...
        "counts": {
            ...
            "underlying_unique_control_trajectories": underlying_unique,

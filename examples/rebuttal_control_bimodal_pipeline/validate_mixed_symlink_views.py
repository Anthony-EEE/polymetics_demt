#!/usr/bin/env python3
"""Independently validate all Control-Bimodal symlink views against frozen Control data."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--output", type=Path, default=CONTROL_BIMODAL_ROOT / "mixed_raw_validation_report.json"
    )
    ...
    for b_index, participant_id in enumerate(PARTICIPANTS, start=1):
        ...
        for mixed_index in range(60):
            ...
            if Path(raw_target_text).is_absolute() or Path(result_target_text).is_absolute():
                ...
                continue
            try:
                raw_resolved = assert_within(raw_link, CONTROL_RAW_ROOT)
                result_resolved = assert_within(result_link, CONTROL_RAW_ROOT)
            ...
            expected_raw = CONTROL_RAW_ROOT / source / f"demo_{demo}"
            expected_result = CONTROL_RAW_ROOT / source / f"demo_{demo:02d}.json"
            ...
            if (
                result.get("group") != CONTROL_GROUP
                ...
                participant_errors.append(f"{mixed_index}:invalid_control_success_metadata")

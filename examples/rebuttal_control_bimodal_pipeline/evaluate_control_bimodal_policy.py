#!/usr/bin/env python3
"""Evaluate one Control-Bimodal policy twice on each of the frozen ten Target specs."""
...
FROZEN_SPEC_SHA256 = "10dbbe16da06b4013899e350fece05687abc8c45471b584ac5a2473a0e486dad"
POLICY_SAMPLING_SEED_BASE = 20260807
...
from rebuttal_control_bimodal_pipeline.control_bimodal_protocol import (  # noqa: E402
    CONTROL_BIMODAL_SUCCESS_CRITERION,
    CONTROL_BIMODAL_XY_TOLERANCE,
    ControlBimodalObstacleTransportSim,
)
...
def write_json_exclusive(path, payload):
    ...
    if path.exists() or path.is_symlink():
        raise FileExistsError(f"Rollout output appeared during write: {path}")
...
def policy_sampling_seed(participant, eval_case_id, repeat_index):
    return (
        POLICY_SAMPLING_SEED_BASE
        + int(participant[2:]) * 10000
        + int(eval_case_id) * 10
        + int(repeat_index)
    )


def next_execution_index(participant_dir, eval_case_id, repeat_index):
    stem = f"case_{eval_case_id:02d}_repeat_{repeat_index}_execution_"
    paths = list((participant_dir / "videos").glob(f"{stem}*.mp4"))
    paths += list(
...
def run_case(
    ...
    sampling_seed = policy_sampling_seed(participant, eval_case_id, repeat_index)
    execution_index = next_execution_index(participant_dir, eval_case_id, repeat_index)
    video_path = participant_dir / "videos" / (
        f"case_{eval_case_id:02d}_repeat_{repeat_index}_execution_{execution_index}.mp4"
    )
    spec = case["spec"]
    ...
    try:
        ...
        # Preserve identical environment/spec and point-cloud streams across repeats;
        # change only the deterministic diffusion-policy sampling stream.
        set_all_seeds(sampling_seed)
        ...
        return {
            ...
            "policy_sampling_seed_rule": (
                "20260807 + numeric_CB_index*10000 + eval_case_id*10 + repeat_index"
            ),
            ...
            "config_sha256": checkpoint_record["config_sha256"],
        ...
        write_json_exclusive(
            ...
            / (
                f"case_{eval_case_id:02d}_repeat_{repeat_index}_execution_"
                f"{execution_index}.json"
            ),
            record,
        )
        raise
...
def main():
    ...
    expected_output_root = (CONTROL_BIMODAL_ROOT / "policy_rollouts").resolve()
    if args.output_root.resolve() != expected_output_root:
        raise ValueError(f"Formal rollout output root must be {expected_output_root}")
    ...
    if spec_manifest.get("group") != TARGET_GROUP or len(spec_manifest.get("eval_cases", [])) != 10:
        raise ValueError("Invalid frozen Target paired rollout manifest")
    if (
        checkpoint_manifest.get("group") != GROUP
        or checkpoint_manifest.get("participant_order") != PARTICIPANTS
        or checkpoint_manifest.get("uniform_training_protocol_sha256")
        != EXPECTED_NORMALIZED_TRAINING_PROTOCOL_SHA256
    ):
        raise ValueError("Invalid Control-Bimodal selected-checkpoint manifest")
    model_validation_path = Path(checkpoint_manifest["model_validation_report"])
    if (
        not model_validation_path.is_file()
        or sha256(model_validation_path)
        != checkpoint_manifest["model_validation_report_sha256"]
        or load_json(model_validation_path).get("passed") is not True
    ):
        raise ValueError("Control-Bimodal model validation report is missing or changed")
    frozen_task = spec_manifest.get("frozen_task", {})
    if (
        frozen_task.get("target_xy_tolerance_m") != CONTROL_BIMODAL_XY_TOLERANCE
        or frozen_task.get("success_criterion") != CONTROL_BIMODAL_SUCCESS_CRITERION
        or frozen_task.get("success_protocol_scope") != TARGET_GROUP
        ...
        raise ValueError("Frozen task thresholds differ from the active Control-Bimodal protocol")
    ...
    if sha256(Path(shared_task_record["target_protocol_path"])) != shared_task_record["target_protocol_sha256"]:
        raise ValueError("Frozen Target success protocol changed after rollout specs were frozen")
    ...
    sim = ControlBimodalObstacleTransportSim(gui=False, output_dir=None, sample_hz=args.sample_hz)
    ...
    try:
        for case in spec_manifest["eval_cases"]:
            ...
            for repeat_index in (0, 1):
                result_path = participant_dir / "rollouts" / (
                    f"rollout_case_{eval_case_id:02d}_repeat_{repeat_index}.json"
                )
                expected_sampling_seed = policy_sampling_seed(
                    args.participant, eval_case_id, repeat_index
                )
                if result_path.exists():
                    ...
                    if (
                        ...
                        or existing.get("repeat_index") != repeat_index
                        or existing.get("policy_sampling_seed") != expected_sampling_seed
                        ...
                        or existing.get("config_sha256") != checkpoint_record["config_sha256"]
                    ):
                ...
                print(
                    f"participant={args.participant} case={eval_case_id} repeat={repeat_index} "
                    f"route={result['route_behavior']['realised_route']} success={result['success']} "
                    f"termination={result['termination_reason']} "
                    f"target_error={result['target_xy_error']:.5f} "
    ...
    result_keys = {(row["eval_case_id"], row["repeat_index"]) for row in results}
    if len(results) != 20 or result_keys != {(case, repeat) for case in range(10) for repeat in (0, 1)}:
        raise RuntimeError("Participant did not produce exactly two valid results per paired spec")
    route_counts = {
        route: sum(row["route_behavior"]["realised_route"] == route for row in results)
    ...
    summary = {
        ...
        "protocol": {
            ...
            "scripted_transport_used": False,
            "repeats_per_eval_case": 2,
            "repeat_difference": "deterministic diffusion-policy sampling seed only",
            "policy_sampling_seed_rule": (
                "20260807 + numeric_CB_index*10000 + eval_case_id*10 + repeat_index"
            ),
            "matched_common_random_numbers_with_Bxx": True,
        },
        "frozen_inputs": frozen_inputs,
        "valid_rollout_count": 20,
        "success_count": sum(int(row["success"]) for row in results),
        "overall_success_rate": sum(int(row["success"]) for row in results) / 20.0,
        "route_counts": route_counts,
        "rollouts": results,
    }
    write_json_exclusive(summary_path, summary)
    print(json.dumps({"participant": args.participant, "success_count": summary["success_count"], "overall_success_rate": summary["overall_success_rate"], "route_counts": route_counts}, indent=2))

#!/usr/bin/env python3
"""Run and post-check one frozen Control-Bimodal participant training configuration."""
...
def main():
    ...
    parser.add_argument("--participant", required=True, choices=[f"CB{i:02d}" for i in range(1, 11)])
    ...
    if (
        ...
        or manifest.get("uniform_training_protocol_sha256")
        != EXPECTED_NORMALIZED_TRAINING_PROTOCOL_SHA256
        or not manifest.get("bimodal_matched_scientific_protocol", {}).get("matched")
    ):
        raise ValueError("Training manifest does not match the frozen Bimodal protocol")
    ...
    hdf5_record = manifest["source_hdf5"][participant]
    hdf5_path = Path(hdf5_record["path"])
    if not hdf5_path.is_file() or sha256(hdf5_path) != hdf5_record["sha256"]:
        raise ValueError(f"Frozen source HDF5 hash mismatch for {participant}")
    hdf5_validation = manifest["hdf5_validation_report"]
    hdf5_validation_path = Path(hdf5_validation["path"])
    if (
        not hdf5_validation_path.is_file()
        or sha256(hdf5_validation_path) != hdf5_validation["sha256"]
    ):
        raise ValueError("Frozen Control-Bimodal HDF5 validation report changed")
    if Path(config["train"]["data"][0]["path"]).resolve() != hdf5_path.resolve():
        raise ValueError("Training config does not reference its frozen participant HDF5")

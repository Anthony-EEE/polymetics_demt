#!/usr/bin/env python3
"""Freeze the uniform Control-Bimodal participant-policy training protocol.
...
SELECTED_EPOCH = 40
TEMPLATE = BIMODAL_MODELS_ROOT / "configs/B01.json"
...
def canonical_protocol(config):
    ...
    value["train"]["output_dir"] = "<GROUP_RUNS_ROOT>"
    return value
...
def main():
    ...
    if validation.get("counts", {}).get("trajectory_memberships") != 600:
        raise ValueError("HDF5 validation did not cover exactly 600 trajectory memberships")
    if validation.get("counts", {}).get("loader_smokes") != 20:
        raise ValueError("HDF5 validation did not pass all 20 real loader smokes")
    ...
    bimodal_training_path, expected_bimodal_training_hash = FROZEN_INPUTS[
        "bimodal_training_manifest"
    ]
    if sha256(bimodal_training_path) != expected_bimodal_training_hash:
        raise ValueError("Frozen existing Bimodal training manifest hash changed")
    bimodal_training = load_json(bimodal_training_path)
    bimodal_b01_config_path = Path(bimodal_training["configs"]["B01"]["path"])
    if sha256(bimodal_b01_config_path) != bimodal_training["configs"]["B01"]["sha256"]:
        raise ValueError("Frozen Bimodal B01 config hash changed")
    if args.template.resolve() != bimodal_b01_config_path.resolve():
        raise ValueError("Formal preparation must use the frozen existing Bimodal B01 config")
    bimodal_scientific_protocol_sha256 = canonical_hash(
        canonical_protocol(load_json(bimodal_b01_config_path))
    )
    if bimodal_scientific_protocol_sha256 != EXPECTED_NORMALIZED_TRAINING_PROTOCOL_SHA256:
        raise ValueError("Existing Bimodal normalized training protocol hash changed")

    validated_hdf5 = {}
    for participant_id in PARTICIPANTS:
        hdf5_path = hdf5_root / f"{GROUP}_{participant_id}_d60_seed20260807_gap2.hdf5"
    ...
    if next(iter(protocol_hashes)) != bimodal_scientific_protocol_sha256:
        raise RuntimeError(
            "Control-Bimodal scientific training protocol differs from frozen Bimodal after "
            "normalizing experiment name, dataset path, and group run root"
        )
    ...
    manifest = {
        ...
        "bimodal_matched_scientific_protocol": {
            "matched": True,
            "normalized_fields": [
                "experiment.name",
                "train.data[0].path",
            ...
            "sha256": bimodal_scientific_protocol_sha256,
            "bimodal_training_manifest": {
                "path": str(bimodal_training_path.resolve()),
                "sha256": expected_bimodal_training_hash,
            },
            "bimodal_reference_config": {
                "path": str(bimodal_b01_config_path.resolve()),
                "sha256": sha256(bimodal_b01_config_path),
            },
        },
        ...
        "software": {
            ...
            "train_entrypoint": {
                ...
                "exception_exit_caveat": (
                    "entrypoint catches training exceptions; Control-Bimodal runner additionally validates completion artifacts"
                ),

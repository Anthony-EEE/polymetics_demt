#!/usr/bin/env python3
"""Validate ten frozen Control-Bimodal checkpoints and run load/inference smokes."""
...
from common import EXPECTED_NORMALIZED_TRAINING_PROTOCOL_SHA256, verify_frozen_inputs
...
def main():
    ...
    errors = []
    if (
        frozen.get("group") != GROUP
        or frozen.get("participant_order") != PARTICIPANTS
        or frozen.get("uniform_training_protocol_sha256")
        != EXPECTED_NORMALIZED_TRAINING_PROTOCOL_SHA256
        or not frozen.get("bimodal_matched_scientific_protocol", {}).get("matched")
    ):
        errors.append("training_manifest_not_exact_bimodal_matched_protocol")
    records = {}
    for participant in PARTICIPANTS:
        try:
            ...
            status = statuses[-1]
            if status.get("subprocess_exit_code") != 0 or status.get("errors") != []:
                raise ValueError("selected training status is not an error-free zero-exit run")
            slurm = status.get("slurm", {})
            expected_task = int(participant[2:])
            if (
                int(slurm.get("array_task_id", -1)) != expected_task
                or not slurm.get("array_job_id")
                or slurm.get("node_list") == "erc-hpc-comp223"
            ):
                raise ValueError("selected training status has invalid Slurm ownership")
            slurm_stderr = (
                models_root
                / "slurm_logs"
                / f"train_{slurm['array_job_id']}_{slurm['array_task_id']}.err"
            )
            if not slurm_stderr.is_file() or slurm_stderr.stat().st_size != 0:
                raise ValueError("selected training task Slurm stderr is missing or nonempty")
            ...
            validation_epochs = [
                int(value) for value in re.findall(r"Validation Epoch ([0-9]+)", log_text)
            ]
            if validation_epochs != list(range(1, 41)):
                raise ValueError(
                    f"training log validation epochs are incomplete: {validation_epochs}"
                )
            if (
                log_text.count("Training completed all 40 epochs.") != 1
                or log_text.count("finished run successfully!") != 1
            ):
                raise ValueError("training log completion markers are not exact")
            ...
            records[participant] = {
                ...
                "slurm_stderr": {
                    "path": str(slurm_stderr.resolve()),
                    "sha256": sha256(slurm_stderr),
                    "size_bytes": 0,
                },

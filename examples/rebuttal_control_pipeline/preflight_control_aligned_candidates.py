#!/usr/bin/env python3
"""Policy-blind IK preflight for the Target-aligned Control revision."""
...
def candidate_reachability(sim, spec):
    ...
    required = {
        "scripted_ee_initial": np.asarray(
            spec["scripted_ee_initial_position"], dtype=float
        ),
        "pre_grasp": np.asarray([box[0], box[1], 0.16], dtype=float),
        "grasp": np.asarray([box[0], box[1], 0.04], dtype=float),
        "transport_start": np.asarray(
            [box[0], box[1], spec["transport_z"]], dtype=float
        ),
    }
    unreachable_setup = [
        name
...
def main():
    ...
    sim = ControlObstacleTransportSim(gui=False, output_dir=None, sample_hz=8.0)
        ...
        for participant_id in CONTROL_PARTICIPANTS:
            for demo_index in range(30):
                for attempt_index in range(args.max_attempts):
                    ...
                    raise RuntimeError(
                        "No IK-feasible candidate within 30 attempts: "
    ...
    source_paths = {
        "aligned_collector": Path(__file__).resolve().parent
        ...
        json.dumps(
            {
                ...
                "max_first_feasible_attempt": max(
                    row["attempt_index"] for row in accepted
                ),
            },
            sort_keys=True,
        )

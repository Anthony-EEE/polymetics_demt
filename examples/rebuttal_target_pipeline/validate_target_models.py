#!/usr/bin/env python3
"""Validate ten frozen Target checkpoints and run load/inference smokes."""
...
def main():
    ...
    for participant in PARTICIPANTS:
        try:
            ...
            if sha256(checkpoint) != status["checkpoint"]["sha256"]:
                raise ValueError("checkpoint hash changed after training postcheck")
            if sha256(log_path) != status["log"]["sha256"]:
            ...
            if sha256(hdf5_path) != hdf5_record["sha256"]:
            ...
            records[participant] = {
                ...
                "source_hdf5_sha256": sha256(hdf5_path),
                "config_sha256": frozen["configs"][participant]["sha256"],

#!/usr/bin/env python3
"""Build ten standalone shuffled 60-trajectory Bimodal HDF5 datasets."""
...
def main() -> None:
    ...
    mixed_validation_path = BIMODAL_ROOT / "mixed_raw_validation_report.json"
    ...
    for source_id, record in source_records.items():
        ...
        if actual_hash != record["hdf5"]["sha256"]:
            raise ValueError(f"Frozen Target HDF5 changed for {source_id}: {source_path}")
    ...
    free_bytes = shutil.disk_usage(BIMODAL_ROOT).free
        ...
        for participant_id in PARTICIPANTS:
            ...
            manifest = {
                ...
                "split": {
                    "seed": SPLIT_SEED,
                    "algorithm": (
                        "one python_stdlib_random.Random(seed) stream over B01..B10; "

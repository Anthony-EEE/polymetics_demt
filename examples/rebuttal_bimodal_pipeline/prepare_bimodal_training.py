#!/usr/bin/env python3
"""Freeze the uniform Bimodal participant-policy training protocol.
...
def main():
    ...
    target_training_path, expected_target_training_hash = FROZEN_INPUTS[
        "target_training_manifest"
    ]
    if sha256(target_training_path) != expected_target_training_hash:
        raise ValueError("Frozen Target training manifest hash changed")
    target_training = load_json(target_training_path)
    target_t01_config_path = Path(target_training["configs"]["T01"]["path"])
    if sha256(target_t01_config_path) != target_training["configs"]["T01"]["sha256"]:
        raise ValueError("Frozen Target T01 config hash changed")
    target_scientific_protocol_sha256 = canonical_hash(
        canonical_protocol(load_json(target_t01_config_path))
    )
    ...
    if next(iter(protocol_hashes)) != target_scientific_protocol_sha256:
        raise RuntimeError(
            "Bimodal scientific training protocol differs from frozen Target after "
    ...
    manifest = {
        ...
        "target_matched_scientific_protocol": {
            "matched": True,
            "normalized_fields": [
                "experiment.name",
                "train.data[0].path",
            ...
            "sha256": target_scientific_protocol_sha256,
            "target_training_manifest": {
                "path": str(target_training_path.resolve()),
                "sha256": expected_target_training_hash,
            },
            "target_reference_config": {
                "path": str(target_t01_config_path.resolve()),
                "sha256": sha256(target_t01_config_path),
            },
        },
        "configs": config_rows,
        ...
        "software": {
            ...
            "train_entrypoint": {
                ...
                "exception_exit_caveat": (
                    "entrypoint catches training exceptions; Bimodal runner additionally validates completion artifacts"
                ),

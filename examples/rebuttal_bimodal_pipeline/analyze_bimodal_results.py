#!/usr/bin/env python3
"""Validate 200 Bimodal rollouts and create route-choice/success statistics."""
...
# Repeated scripted grasps can differ at PyBullet's contact-solver precision
# even when the sampled task, reset state, and scripted commands are identical.
# These bounds are deliberately tiny relative to the task geometry and record
# physical equivalence without pretending the resulting floats are bitwise
# identical.
REPEAT_TRANSLATION_TOLERANCE_METRES = 1e-5
REPEAT_ORIENTATION_TOLERANCE_DEGREES = 0.01
OUTPUT_NAMES = [
    "final_results.json",
...
def policy_sampling_seed(participant, case_id, repeat_index):
    return 20260807 + int(participant[1:]) * 10000 + int(case_id) * 10 + int(repeat_index)
...
def plot_bytes(participant_rows, pooled):
    import matplotlib
...
def main():
    ...
    root = args.bimodal_root.resolve()
    ...
    if existing:
        raise FileExistsError(f"Refusing to overwrite final analysis: {existing}")
    ...
    required = {
        ...
        "hdf5_recovery": root / "diagnostics/hdf5_attempt_1_recovery.json",
    }
    ...
    if not training_manifest.get("target_matched_scientific_protocol", {}).get("matched"):
        errors.append("training_protocol_not_target_matched")
    ...
    participant_dirs = (
        sorted(path.name for path in rollout_root.iterdir() if path.is_dir() and path.name.startswith("B"))
        ...
        for case in range(10):
            ...
            for key, value in distances.items():
                ...
                tolerance = (
                    ...
                    else REPEAT_TRANSLATION_TOLERANCE_METRES
                )
                if value > tolerance:
                    errors.append(
    ...
    if errors:
        raise RuntimeError("Final Bimodal validation failed:\n" + "\n".join(errors))
    ...
    final_results = {
        ...
        "inferential_unit": "Bimodal participant-level policy",
        ...
        "data_reuse_limitation": {
            ...
            "underlying_unique_target_trajectories": 300,
    ...
    summary_lines = [
        "Bimodal route and success summary",
        *(
        ...
        "The 600 memberships reuse 300 underlying Target trajectories with unequal multiplicity.",
    ]
    helper_records = {
    ...
    experiment_manifest = {
        ...
        "hdf5_infrastructure_recovery": load_json(required["hdf5_recovery"]),
        ...
        "isolation_audit": {
            "bimodal_owned_roots_only": True,
            "target_frozen_inputs_reverified_by_hash_at_each_gate": True,
            "target_or_control_job_action_by_bimodal_executor": False,
            "control_array_36371239_was_queried_read_only_and_never_modified": True,
        },
        "statistics_contract": {
    ...
    planned = {
        ...
        "route_success_summary.png": plot_bytes(participant_rows, pooled),
    ...
    validation_report = {
        ...
        "cross_checks": {
            ...
            "all_rollouts_reference_correct_bimodal_checkpoint_hash": True,
            ...
            "paired_repeat_physical_starts_within_recorded_tolerance": True,
        },
        "paired_repeat_physical_equivalence": repeat_equivalence,

...
from analyze_tro_position_composition import same_nonzero_sign_as_mean
from eval_abla1_trained_policies import (
    ...
    accepted_start_manifest_condition_orders,
    active_conditions,
)
...
def test_composition_profile_is_independent_and_complete():
    ...
    assert accepted_start_manifest_condition_orders(args) == {
        TRO_POSITION_COMPOSITION_ORDER,
        TRO_POSITION_STAGE2_ORDER,
    }


def test_noncomposition_profiles_do_not_accept_legacy_condition_order():
    args = argparse.Namespace(experiment_profile="tro_position_stage2")
    assert accepted_start_manifest_condition_orders(args) == {
        TRO_POSITION_STAGE2_ORDER
    }
...
def test_formal_rollout_pairing_scopes_match_frozen_surface_semantics():
    assert {
        composition.rollout_pairing_group("common_absolute", condition)
        for condition in composition.CONDITION_ORDER
    } == {"all_conditions"}
    assert {
        composition.rollout_pairing_group("rmax40", condition)
        for condition in composition.CONDITION_ORDER
    } == {"START15", "START25", "START35"}


def test_analysis_sign_count_is_builtin_int_and_json_serializable():
    count = same_nonzero_sign_as_mean(
        [np.float64(-0.2), np.float64(0.1), np.float64(0.2)], 0.1
    )
    assert count == 2
    assert type(count) is int
    assert json.loads(json.dumps({"count": count})) == {"count": 2}
